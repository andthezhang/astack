...
name: my-skill
description: "..."
source_docs: [docs/architecture/ARCHITECTURE.md, docs/architecture/decisions/thing.md, CONTEXT.md]
...
1. Classify the root cause:
   - `skill-missing` — no skill triggered for the situation. Sharpen an existing skill or record an orphan lesson; do not create a new skill by default.
   - `skill-wrong` — a skill triggered but its description or body led the agent astray. Sharpen the trigger or correct the body.
   - `doc-missing` — the source principle isn't written down anywhere. Update the right durable home first (`CONTEXT.md`, architecture docs, decision records, issues, or agent rules), then materialize into an existing skill.
2. Write a `lessons.md` entry. Location:
   - If a skill owns the territory → `<skill>/lessons.md` (per-skill).
   - If no skill owns it yet → `.astack/pending-lessons.md` (orphan queue).
...
1. Read `git log --since="1 month ago" -- skills/ CONTEXT.md AGENTS.md docs/architecture docs/agents docs/issues` for recent churn.
2. Read every `<skill>/lessons.md` and `.astack/pending-lessons.md`.
3. Read the latest `docs/issues/active/*drift*.md` local issue if present.
4. For each skill, propose one of:
   ...
   - **delete** — the skill no longer has a durable basis; the underlying principle went away.
5. Write proposals to the external issue tracker if configured; otherwise write `docs/issues/active/YYYY-MM-DD-skill-audit.md`. Hand off application to the user or astack-compound.

### Mode 3: `drift` — daily cron

Finds divergence between recent commits and the docs skills cite.

1. Run `scripts/drift-check.ts` to harvest `(commit-diff, candidate-doc)` pairs from the last 24h. The script uses these candidate sources:
   - current implemented picture (`docs/architecture/ARCHITECTURE.md`) when present
   - path-slug match (a file `src/auth/session.ts` pairs with `docs/architecture/decisions/auth-session.md` if one exists)
   - explicit `source_docs:` backrefs from skills when they point at `docs/architecture/ARCHITECTURE.md` or `docs/architecture/decisions/*.md`
...
3. Append `yes` findings to the external tracker if configured; otherwise write/update `docs/issues/active/YYYY-MM-DD-drift-report.md` with the commit SHA, doc path, and the one-sentence reasoning.
...
| Orphan lessons | `.astack/pending-lessons.md` | Lessons with no owning skill yet |
| Skill body | `<skill>/SKILL.md` | Canonical view — graduated rules live here |
| Source docs | `CONTEXT.md`, `AGENTS.md`, `docs/agents/`, `docs/architecture/` | Source of truth — skills materialize from these |
| Drift report | external tracker or `docs/issues/active/YYYY-MM-DD-drift-report.md` | Harvest output; audit input |
| Audit issue | external tracker or `docs/issues/active/YYYY-MM-DD-skill-audit.md` | Curation proposals |
...
- `astack-docs` — structure and linting of the `docs/` tree. `astack-skills` writes into `docs/issues/` when no external tracker is configured; those paths must stay on the allowlist.
...
- State which files were written (`lessons.md`, `.astack/pending-lessons.md`, drift issue, audit issue).
